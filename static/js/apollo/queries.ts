...
const FACTORY_ADDRESS = "0x5A67D6d1e1c36dcA265ca9b9BDA0e3Eb5Af9B634";

const BUNDLE_ID = '1'

export const SUBGRAPH_HEALTH = gql`
  query health {
    indexingStatusForCurrentVersion(subgraphName: "moondogeswap/exchange") {
      synced
      health
      chains {
        chainHeadBlock {
...
export const BNB_PRICE = (block?:any) => {
...
export const GLOBAL_CHART = gql`
  query moondogeDayDatas($startTime: Int!, $skip: Int!) {
    moondogeDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block?:any) => {
  const queryString = ` query moondogeFactories {
      moondogeFactories(
...
export const ALL_PAIRS = (block) => {
  ...
  query pairs($skip: Int!) {
    pairs(first: 100, skip: $skip, orderBy: trackedReserveBNB, orderDirection: desc, where: { createdAtBlockNumber_gte: 5619965 }) {
      id
      token0 {
        id
        symbol
...
export const PAIRS_CURRENT = gql`
  query pairs {
    pairs(first: 100, where: { createdAtBlockNumber_gte: 5619965 }, orderBy: trackedReserveBNB, orderDirection: desc) {
      id
    }
  }
`

export const PAIR_DATA = (pairAddress, block?:any) => {
  const queryString = `
    ${PairFields}
    query pairs {
...
export const TOKEN_DATA = (tokenAddress, block?:any) => {
  const queryString = `
    ${TokenFields}
    query tokens {
